...
const baseurl = "https://bardie.cyclic.app";
...
class Bardie {
  ...
  async question({ ask }) {
    ...
    try {
      const response = await axios.post(baseurl + "/api/conversation", { ask }, {
        headers: {
  ...
  async questionWithImage({ ask, image }) {
    ...
    try {
      const response = await axios.post(baseurl + "/api/conversation/image", { ask, image }, {
        headers: {
          "Content-Type": "application/json",
          "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
          Origin: baseurl,
